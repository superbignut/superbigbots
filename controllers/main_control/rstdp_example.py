...
class TestNet(spaic.Network):
    def __init__(self):
        ...
        self.connection1 = spaic.Connection(self.input, self.layer1, link_type='full',
                                              weight=(np.random.rand(label_num, 784) * 0.3)) # 100 * 784
        ...
        self.connection3 = spaic.Connection(self.layer2, self.layer1, link_type='full', 
                                            weight=(np.ones((label_num, label_num)) - (np.diag(np.ones(label_num)))) * (-4)) # 起到一个抑制的作用，除了1-1的前一个，侧向抑制，并起到竞争的效果

        # Learner
        self._learner = Learner(algorithm='rstdpet', trainable=self.connection1, run_time=run_time, lr=0.1) # 这里也只是训练 从输入到第一层的连接，其余层不变

        self.reward = spaic.Reward(num=label_num, dec_target=self.layer1, coding_time=run_time, coding_method='environment_reward', dec_sample_step=256) # 这两个参数不太明白
        # reward 被当作一种特殊的解码器
