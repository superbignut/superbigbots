...
class RSTDP(Learner):
    ...
    def weight_update(self, weight, eligibility, reward):
        ...
        with torch.no_grad():
            if len(reward.shape) > 1 and reward.shape[1] == eligibility.shape[0]: # reward = 1* 10 trace = 10 * 100
                reward = reward.transpose(1, 0) # 10 * 1
                reward = reward.repeat(1, eligibility.shape[1]) # 把奖励进行 repeat 10 * 100 
            # 如果本身就是常量的话就直接乘就好了
            # print(reward)
            weight.add_(self.learning_rate * reward * eligibility) # 这里没乘dt
        return weight
    ...
    def build(self, backend):
        ...
        for conn in self.trainable_connections.values():
            ...
            # eligibility = torch.matmul(post.transpose(1, 0), p_plus) + torch.matmul(p_minus.transpose(1, 0), pre) # 这个是论文中的简化版
...
class RSTDPET(Learner):
    ...
    def build(self, backend):
        ...
        for conn in self.trainable_connections.values():
            ...
            # pre = pre.view(-1)
            # post = post.view(-1)
            # p_plus *= np.exp(-dt / tau_plus) # 衰减
            # p_plus += A_plus * pre # + A * 脉冲到来
            # p_minus *= np.exp(-dt / tau_minus) # 衰减
            # p_minus += A_minus * post # + 系数 * 脉冲到来
            # eligibility = torch.ger(post, p_plus) + torch.ger(p_minus, pre)
            # eligibility_trace *= np.exp(-dt / tau_e_trace) # trace 也会有一层的衰减 公式 35 所以这个RstdpET是完整版
