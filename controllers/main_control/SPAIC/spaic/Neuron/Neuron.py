...
class LIFModel(NeuronModel):
    ...
    # 所以这就是一个简单的LIF 比LIF 加了一个静息电位和一个 \theta 的抑制作用 
    # theta 会增大或减小
    """
    LIFSTDPEX model:
    V(t) = decay_v * (v - v_rest) + v_rest + I^n[t] # 这个与积分形式是等价的 ： \tao * dV/ dt = (E_res - V) 
    I^n[t] = V0 * Isyn^n[t]  #V0 = 1 # 
    theta(t) = decay_th * theta[t-1] # decay_th ；抑制效果会不断的衰减，引入是为了防止 某个神经元形成主导地位
    if v >= (vth + theta) then s_out = 1; else s_out = 0; # v_th 是 脉冲的 threshold
    

    Reset:
    V(t) = s_out * v_reset + (1 - s_out) * v; # 这里没有脉冲也不会衰减
    theta = theta + s_out * th_inc #  每次释放后，theta 会增大，导致下次释放变难，抑制效果
    O^n[t] = spike_func(V^n[t-1])
    """
    def __init__(self, **kwargs):
...
class LIFSTDPEXModel(NeuronModel):
    ...
    def __init__(self, **kwargs):
        ...
        self._operations.append(('O', 'threshold', 'Vtemp', 'Vth_theta')) # VTEMP 如果超过阈值则发放

        self._operations.append(('Resetting1', 'var_mult', 'Vreset', 'O[updated]')) # updated 表示更新之后的电压吗
        self._operations.append(('Resetting2', 'var_mult', 'Vtemp', 'O[updated]'))

        self._operations.append(('Resetting3', 'minus', 'Vtemp', 'Resetting2'))

        self._operations.append(('V', 'add', 'Resetting1', 'Resetting3')) # V(t) = s_out * v_reset + (1 - s_out) * v; 

        self._operations.append(('Resetting_theta', 'var_mult', 'O[updated]', 'th_inc'))
        self._operations.append(('theta[stay]', 'add', 'theta_temp', 'Resetting_theta')) # theta 更新
...
class ALIFSTDPEXModel(NeuronModel):
    ...
    def __init__(self, **kwargs):
        ...
        self._variables['I'] = 0.0
        self._variables['V'] = 0.0 # 与LIFSTDPEXModel的区别就是 数值不同吗
        self._variables['O'] = 0.0
        self._variables['Isyn'] = 0.0
        self._variables['theta[stay]'] = 0.0
...
class LIFSTDPIHModel(NeuronModel):
    ...
    这个相比  STDPEX 没有 \theta 
    LIF model:
    V(t) = decay_v * (v - v_rest) + v_rest + I^n[t] # 输出
    I^n[t] = V0 * Isyn^n[t]  #V0 = 1 # 输入
