...
class Node(Assembly):
    ...
    def __call__(self, data=None):
        ...
        elif isinstance(self, Decoder): # output(label) 走到这里但是source 不会被用到
            if isinstance(data, np.ndarray):
                self.source = data
            elif isinstance(data, torch.Tensor):
                self.source = data
...
class Decoder(Node):
    '''
        ...
        2. 'first_spike': First_Spike # 这其实有最先spike 的解码方法
        3. 'time_spike_counts': TimeSpike_Counts
        4. 'time_softmax': Time_Softmax
        5. 'final_step_voltage': Final_Step_Voltage # 这个需要搭配 if使用，也就是不发脉冲
    '''
    ...
    def get_output(self, output):
        ...
        if self.index >= self.time_step:
            if self.sim_name == 'pytorch':
                self.predict = self.torch_coding(self.records, self.source, self.device) # 这里的第二个参数在Decoder的子类中都没有使用
            else:
...
class Reward(Node):
    ...
    def get_reward(self, output=np.empty(0)):
        ...
        if (self.index % self.dec_sample_step) == 0: # 到了取奖励的时间周期
            self.index = 0
            shape = list(output.shape)
