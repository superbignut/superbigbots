...
class PopulationRate_Action(Action):
    ...
    def torch_coding(self, record, target, device):
        ...
        assert record.shape[2] % self.num == 0, ( # 这里允许 被解码层的神经元的数量是 动作器的num 数量的倍数?
            f"Output layer size of {record.shape[2]} is not divisible by action space size of"
            f" {self.num}."
        )

        pop_size = int(record.shape[2] / self.num) # 可以被分成这么多类
        ...
        if spike_num == 0:
            ...
            pop_spikes = torch.tensor(
                [
                    record[:, :, (i * pop_size): (i * pop_size) + pop_size].sum() # 计算每组的脉冲数量
            ...
            action = torch.multinomial((pop_spikes / spike_num).view(-1), 1)[0].item() # 使用 multinomial 进行采样
        return action
...
class Highest_Spikes_Action(Action):
    ...
    def torch_coding(self, record, target, device):
        ...
        assert (
           record.shape[2] == self.num # 维度必须相等
        ), "Output layer size is not equal to the size of the action space."
...
class First_Spike_Action(Action):
    ...
    def torch_coding(self, record, target, device):
        ...
        spikes = record.squeeze().nonzero() # nonzero 返回不是0的index
        if spikes.shape[0] == 0:
            action = torch.randint(low=0, high=1, size=(1,))[0]
        else:
            action = spikes[0, 1] # 为什么不是 0，0呢
